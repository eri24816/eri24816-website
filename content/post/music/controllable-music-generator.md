...
The reason I assume that the AI is only an auxiliary providing inspirations and that it's still human composing music is that the AI has yet no ability to percept the global structure (I'm currently working on a new model that considers the global structure). You have to fix parts it generate strangely, and sometimes you need to specify chords and velocities for each bar to 
